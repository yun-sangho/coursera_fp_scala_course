...
object Anagrams extends AnagramsInterface {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    w.toLowerCase.toSeq.groupBy(identity)
      .map(x => (x._1, x._2.length)).toList.sorted
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences =
    y.foldLeft(x.toMap)((acc, cur) =>
      cur match {
        case (curChar, curCount) =>
          if (curCount == acc(curChar)) acc - curChar
          else acc updated(curChar, acc(curChar) - curCount)
      }
    ).toList.sorted
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def findAnagram(occurrences: Occurrences): List[Sentence] =
      if (occurrences.isEmpty) List(Nil)
        ...
        rest <- findAnagram( subtract(occurrences, combination) )
      } yield word :: rest
