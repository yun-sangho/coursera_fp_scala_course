...
object GoogleVsApple {
  ...
  def filterByWordList(wordList: List[String]): TweetSet =
    TweetReader.allTweets.filter(tweet => wordList.exists(s => tweet.text.contains(s)))

  lazy val googleTweets: TweetSet = filterByWordList(google)
  lazy val appleTweets: TweetSet = filterByWordList(apple)
